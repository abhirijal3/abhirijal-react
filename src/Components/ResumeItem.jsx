...
const ResumeItem = (props) => {
    ...
    return (
        <div>
            <div className='my-4'>
                <div onClick={() =>{console.log("CLICKED")}} style={{ 'cursor': 'pointer' }} data-bs-toggle="collapse" data-bs-target={"#" + uniqueId} aria-expanded="false" aria-controls="collapseWidthExample">
                    <div className='d-flex justify-content-between'>
                        <div>
                            <div>
                                <span className='font-karla fw-medium text-gray' style={{ 'fontSize': '23px' }}>{props.data.title}</span>
                            </div>
                        </div>
                        <div>
                        ...
                        <div>
                            <div className='d-flex'>
                                <div className='d-flex align-items-center me-3'>
                                    ...
                                    <i className={`fa-solid ${(props.data.type === "school") ? "fa-graduation-cap" : "fa-building"} me-2`}></i>
                                    <span className='font-manrope fw-semibold text-gray' style={{ 'fontSize': '14px' }}>{props.data.organization}</span>
                                </div>
                                <div className='d-flex align-items-center'>
                                    ...
                                    <span className='font-manrope fw-semibold text-gray' style={{ 'fontSize': '14px' }}>{props.data.location}</span>
                                </div>
                            </div>
                        </div>
                        <div>
                            <span className='font-manrope fw-semibold text-gray' style={{ 'fontSize': '13px' }}>{props.data.date}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="collapse" id={uniqueId}>
                    <div className="card card-body" style={{ 'border': 'none' }}>
                        {(props.data.description.length > 0) ?
                            <ul>
                                {props.data.description.map((eachItem, index) => {
                                    return (
                            ...
                            : <div></div>}
                        <p className='mb-0'>
                            <b className='me-2'>{(props.data.type === "school") ? "Relevant Coursework:" : "Skills:"}</b>
                            {props.data.skills}
                        </p>
                    </div>
                </div>
